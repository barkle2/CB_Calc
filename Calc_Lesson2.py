...
for num_text in num_list:
    ...
    tk.Button(num_frame, text=num_text, width=5, command=cmd_tmp).grid(row=row_tmp, column=col_tmp)
...
for op_text in op_list:
    ...
    # Button을 생성하고, row_tmp, col_tmp 위치에 배치
    num_button = tk.Button(op_frame, text=op_text, width=5, command=cmd_tmp)
